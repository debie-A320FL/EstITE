...
for i in range(B):
    ...
    # Generate Surfaces
    mu = 6 + 0.3*AIDS["wtkg"]**2 - np.sin(AIDS["age"])*(AIDS["gender"] + 1) + 0.6*AIDS["hemo"]*AIDS["race"] - 0.2*AIDS["z30"]

    ITE = 1 + 1.5*np.sin(AIDS["wtkg"])*(AIDS["karnof_hi"] + 1) + 0.4*AIDS["age"]**2
...
for name in Results.keys():
    ...
    PD_results.to_csv(basedir + "/ACTG_ThirdDGP/Results/GP_%s_%s.csv" % (B, name), index=False, header=True)
