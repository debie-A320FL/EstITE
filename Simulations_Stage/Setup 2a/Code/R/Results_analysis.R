...
generate_summary_tables <- function(fac_vector = c(0, 0.1, 0.5, 1, 2, 3, 5), num_ACTG = 80, num_GP = 80) {
  ...
  # Loop through each fac value
  for (fac in fac_vector) {
    ...
    mean_train_list[[as.character(fac)]] <- mean_train
    sd_train_list[[as.character(fac)]] <- sd_train
    mean_test_list[[as.character(fac)]] <- mean_test
    sd_test_list[[as.character(fac)]] <- sd_test
  }
...
# Example usage
results <- generate_summary_tables(fac_vector = c(0, 0.1, 0.5, 1, 2, 3, 5))
