...
for size_sample in size_sample_list:
    ...
    for sim in range(B):
        ...
        print(f"\n*** Iteration {sim + 1} - Size sample : {size_sample}")
        ...
        if save_learning_curve and sim < 5:
            fname = f'learning_curves_{sim}_STXM_tr_per_{treatment_percentile}_size_{size_sample}.pdf'
    ...
    df_pehe.to_csv(os.path.join(results_dir, f"pehe_results_N_{size_sample}.csv"), index=False)
    df_time.to_csv(os.path.join(results_dir, f"time_results_N_{size_sample}.csv"), index=False)
    ...
    summary.to_csv(os.path.join(results_dir, f"summary_N_{N}.csv"))