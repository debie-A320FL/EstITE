...
#----------------------------------------------------------------
    def __init__(self, mode="CMGP", backend="gpy",**kwargs):
        """
        Class constructor. 
        ...
        if self.backend not in ['gpy', 'gpytorch']:
            raise ValueError(f"Unsupported backend '{self.backend}'. Choose 'gpy' or 'gpytorch'.")
...
#---------------------------------------------------------------
    ...
    def fit(self, X, Y, W, **fit_kwargs):
        """Fits the model given features X, outcomes Y, and treatment W"""
        if self.backend == 'gpy':
            return self._fit_gpy(X, Y, W)
        else:
            return self._fit_gpytorch(X, Y, W, **fit_kwargs)

    def _fit_gpy(self, X, Y, W):
        """
    ...
    def _fit_gpytorch(self, X, Y, W, lr=0.1, epochs=100, batch_size=None):
        if 1:
            raise ValueError("gpytorch is not yet implemented")
...
#---------------------------------------------------------------
    ...
    def predict(self, X):
        """Predicts mean and variance for given X and treatment W"""
        if self.backend == 'gpy':
            return self._predict_gpy(X)
        else:
            return self._predict_gpytorch(X)

    def _predict_gpy(self, X):
        """
    ...
    def _predict_gpytorch(self, X, W):
        if 1:
            raise ValueError("gpytorch is not yet implemented")
#-----------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------- 
